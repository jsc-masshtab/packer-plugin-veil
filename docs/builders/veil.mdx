---
description: |
  The veil Packer builder is able to create new templates for use with
  ECP VeiL. The builder takes either an ISO or an existing template as it's
...
Type: `veil`
Artifact BuilderId: `packer.veil`

The `veil` Packer builder is able to create new templates for use with
[ECP VeiL](https://veil.mashtab.org). The builder takes either an ISO
...
[ECP VeiL plugin](https://github.com/jsc-masshtab/packer-plugin-veil).
...
- `api_token` (string) - The API token used to sign all API requests. It can also
...
- `http_get_only` (boolean) - Some cloud providers only allow HTTP GET calls
  to their ECP VeiL API. If using such a provider, you need to set this to
  `true` in order for the provider to only make GET calls and no POST calls.

- `eject_iso` (boolean) - If `true` make a call to the ECP VeiL API, after
