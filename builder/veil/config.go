...
type Config struct {
	...
	// The ECP VeiL API endpoint we will connect to. It can
	// also be specified via environment variable VEIL_API_URL, if set.
	ApiUrl string `mapstructure:"api_url" required:"true"`
	// The ECP VeiL API token used to sign all API requests. It can also
	...
	// If `true` make a call to the veil API, after loading image to
...
// Prepare NewConfig parses and validates the given config.
func (c *Config) Prepare(raws ...interface{}) error {
	...
	if c.ApiUrl == "" {
		...
		c.ApiUrl = os.Getenv("VEIL_API_URL")
	}

	if c.ApiToken == "" {
		// Default to environment variable for api_token, if it exists
		c.ApiToken = os.Getenv("VEIL_API_TOKEN")
	}
	...
	if c.ApiToken == "" {
		errs = packersdk.MultiErrorAppend(errs, errors.New("a api_token must be specified"))
	}
