...
func (s *stepPrepareConfig) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	//client := state.Get("client").(*veil.WebClient)
	...
	//	config.Network, _, err = client.Network.GetNetworkID(config.Network, veil.WithProject(config.Project))
	//	if err != nil {
	...
	//			config.SecurityGroups[i], _, err = client.SecurityGroup.GetSecurityGroupID(config.SecurityGroups[i], veil.WithProject(config.Project))
	//			if err != nil {
