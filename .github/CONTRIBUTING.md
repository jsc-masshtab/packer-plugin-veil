# Contributing to ECP VeiL Plugin
...
runtime in order to build packer with the ECP VeiL plugin.
...
## Setting up ECP VeiL plugin for dev

With Go installed, you can already `go get` the ECP VeiL plugin and `make dev` in
...
1. Download the ECP VeiL plugin source (and its dependencies) by running
   `go get github.com/hashicorp/packer-plugin-veil`. This will download the source to
   `$GOPATH/src/github.com/hashicorp/packer-plugin-veil`.

2. When working on the ECP VeiL plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-veil`
   so you can run `make dev` and easily access other files. `make dev` will build the packer-plugin-veil binary and install it under `$HOME/.packer.d/plugins/`.

3. Make your changes to the ECP VeiL plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
   Any compilation errors will be shown when the binaries are rebuilding. If you don't have `make` you can simply run `go build -o packer-plugin-veil` from the project root, and `mv packer-plugin-ECP VeiL ~/.packer.d/plugins/packer-plugin-veil` to install the plugin.

4. After building the ECP VeiL plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-veil/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the ECP VeiL plugin](https://github.com/hashicorp/packer-plugin-veil#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-veil f-my-feature
```
...
3. One of ECP VeiL plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the ECP VeiL plugin
project. After following the steps in "Setting up Go to work on the ECP VeiL plugin":

1. Navigate to the code:

   `cd $GOPATH/src/github.com/hashicorp/packer-plugin-veil`
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-veil.git`
...
Please only apply the minimal vendor changes to get your PR to work. The ECP VeiL plugin
...
If the ECP VeiL Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
